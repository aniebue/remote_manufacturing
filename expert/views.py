#here we import all the neccessary files from Django core and Models
...
#below is where you have all the functions thats will handle everything the expert does in the system


#This is the Login function for expert
def login(request):
...
#Get notification function, which is called from other functions
def get_notification(request):
    notification = Requests.objects.filter(expert_view="0").exclude(Q(request_sender="Expert") | Q(request_type="Reminder")).count()
    return notification

#This function handles calling for the notification every 10 seconds to keep it real time
def get_ajax_notification(request):
    notification = Requests.objects.filter(expert_view="0").exclude(Q(request_sender="Expert") | Q(request_type="Reminder")).count()
    return HttpResponse(notification)

#This is the expert Dashboard for the User(Expert)
...
#Request maintanace function handling the Requests from expert
...
def video_call(request, id):
    ...
    context['call_type'] = 'video_call' if 'video' in request.GET and request.GET['video'] == '1' else 'voice_call'
